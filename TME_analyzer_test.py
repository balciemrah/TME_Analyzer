...
class ImageAnalysis:
    def __init__(self, master):
        ...
        fileMenu.add_command(label="Load Segmentation Map",
                             command=lambda:[fMO.openSeg(self)])
        fileMenu.add_command(label="Load Folder", command=lambda:[fMO.openFolder(self)])
        fileMenu.add_separator()
        fileMenu.add_command(label="Load Workspace",
                             command=lambda:[fMO.LoadWorkspace(self)])
        fileMenu.add_command(label="Load Workspace Directory",
                             command=lambda:[fMO.LoadWorkspaceFolder(self)])
        fileMenu.add_separator()
        fileMenu.add_command(label="Change Channel Setup",
                             command=lambda:[fMO.ChangeChannelSetup(self)])
        ...
        imageMenu.add_command(label="Threshold Foregound",
                              command=lambda:[iMO.ThresholdForeground(self)])
        imageMenu.add_command(label="Tissue Segmentation",
                              command=lambda:[iMO.SegmentDetection(self)])
        imageMenu.add_command(label="Image Filtering",
                              command=lambda:[iMO.FilterImage(self)])
        imageMenu.add_command(label="Fill Holes", command=lambda:[iMO.FillHoles(self)])
        imageMenu.add_command(label="Nucleus Detection",
                              command=lambda:[iMO.NucleusDetection(self)])
        imageMenu.add_separator()
        imageMenu.add_command(label="Filter Image", command=lambda:[iMO.FilterImage(self)])
        imageMenu.add_command(label="ISH/Mask Enumeration",
                              command=lambda:[iMO.MaskEnumeration(self)])
        imageMenu.add_command(label="Tissue Analysis",
                              command=lambda:[iMO.Tissue_analysis(self)])
        imageMenu.add_command(label="Neural Networks Segmentation",
        ...
        dataMenu.add_command(label="Redo Analysis for all",
                             command=lambda:[dMO.RedoAnalysisAll(self)])
        dataMenu.add_command(label="Display Log",
                             command=self.DisplayLog)
        saveMenu = tkinter.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Saving", menu=saveMenu)
        saveMenu.add_command(label="Save Image", command=lambda:[sMO.SaveCurrentImage(self)])
        saveMenu.add_separator()
        saveMenu.add_command(label="Export Data", command=lambda:[sMO.ExportData(self)])
        saveMenu.add_command(label="Save Workplace",
                             command=lambda:[sMO.SaveWorkspace(self)])
        saveMenu.add_command(label="Redo Analysis and Save Workplace",
                             command=lambda:[sMO.SaveWorkspace(self,True)])
        saveMenu.add_command(label="Save Phenotype Image",
                             command=lambda:[sMO.SavePhenos(self)])
        saveMenu.add_separator()
        subSaveMenu = tkinter.Menu(saveMenu, tearoff=0)
        saveMenu.add_cascade(label="Save For All", menu=subSaveMenu)
        subSaveMenu.add_command(label="Save Images",
                                command=lambda:[sMO.SaveAllImages(self)])
        subSaveMenu.add_separator()
        subSaveMenu.add_command(label="Export Data",
                                command=lambda:[sMO.Export_folder_data(self)])
        subSaveMenu.add_command(label="Save Workplaces",
                                command=lambda:[sMO.Export_folder_workspace(self)])
        subSaveMenu.add_command(label="Redo Analysis and Save Workplaces",
                                command=lambda:[
                                    sMO.Export_folder_workspace(self,True)])
        subSaveMenu.add_command(label="Save Phenotype Images",
                                command=lambda:[sMO.SavePhenosAll(self)])
        subSaveMenu.add_separator()
        ...
        loadButton = ttkinter.Button(toolbar, text="Load Image",
                                     command=lambda:[fMO.openFile(self)])
        loadButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        folderButton = ttkinter.Button(toolbar, text="Load Folder",
                                       command=lambda:[fMO.openFolder(self)])
        folderButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        foregorundButton = ttkinter.Button(toolbar,
                                           text="Threshold Foreground",
                                           command=lambda:[iMO.ThresholdForeground(self)])
        foregorundButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        segmentButton = ttkinter.Button(toolbar, text="Tissue Segmentation",
                                        command=lambda:[iMO.SegmentDetection(self)])
        segmentButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        fillHolesButton = ttkinter.Button(toolbar, text="Fill Holes",
                                          command=lambda:[iMO.FillHoles(self)])
        fillHolesButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        nucleusButton = ttkinter.Button(toolbar, text="Nucleus Detection",
                                        command=lambda:[iMO.NucleusDetection(self)])
        nucleusButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        phenotypeButton = ttkinter.Button(toolbar, text="Phenotype Selection",
                                          command=lambda:[dMO.PhenotypeSelection(self)])
        phenotypeButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        exportButton = ttkinter.Button(toolbar, text="Export Data",
                                       command=lambda:[sMO.ExportData(self)])
        exportButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        dataAnalysis = ttkinter.Button(toolbar, text="Data Analysis",
                                       command=lambda:[dMO.DataAnalysis(self)])
        dataAnalysis.pack(side=tkinter.LEFT, padx=2, pady=2)
        toolbar.pack(side=tkinter.TOP, fill=tkinter.X)

        # ***** Status Bar *****
        statusBar = tkinter.Label(master, text="Analyzing Images...", bd=1,
...
#        self.mainWindow = tkinter.Frame(master, width=700)
    ...
    def PerformDefinitions(self, default_setup, show_messages):
        ...
        loadButton = ttkinter.Button(toolbar, text="Clear",
                                      command= lambda: [
                                          text.delete('1.0', tkinter.END)])
        loadButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        folderButton = ttkinter.Button(toolbar, text="Close",
                                        command=self.log_keeping.withdraw)
        folderButton.pack(side=tkinter.LEFT, padx=2, pady=2)
        print_button = ttkinter.Button(toolbar, text="Print params",
                                        command=lambda: [
    ...
    def RedoPhenotyping(self, *a):
        def Procrastinate(popup2,*a):
            ...
            B1.pack()
            B2 = tkinter.Button(popup2, text="Maybe", command=lambda: [
                popupmsg("OK, I'm reanalyzing now."),
                RedoPhenotypingSure(), DestroyTK(popup2),
        ...
        def RedoPhenotypingCheck(*a):
            popup2 = tkinter.Tk()
            popup2.protocol("WM_DELETE_WINDOW", lambda: [
                popupmsg("OK, I'm reanalyzing now."),
                RedoPhenotypingSure(), DestroyTK(popup2),
                popupmsg("Done, thank you for your cooperation.")])
            popup2.wm_title("Redo Analysis?")
            label = tkinter.Label(
                    popup2, text="Your adjustment is expected to change " +
                    "phenotyping and\nwhat you see will not be what you get" +
                    " until you Redo analysis\nwould you like to do that now?")
            label.pack(side="top", fill="x", pady=10)
            B1 = tkinter.Button(
                    popup2, text="Redo Analysis now",
                    command=lambda: [RedoPhenotypingSure(), DestroyTK(popup2)])
            B1.pack()
            B2 = tkinter.Button(popup2, text="I will do it later",
    ...
    def remake_side_window(self):
            ...
            if master_check:
                ...
                for n_i, i in enumerate(self.check_boxes[:-1]):
                    if i.get() == 0:
                        master_check = False
                if master_check:
                    self.check_boxes[-1].trace_remove(*self.check_boxes[-1].trace_info()[0])
                    self.check_boxes[-1].set(1)
                    self.check_boxes[-1].trace_id = self.check_boxes[-1].trace_add('write', checks_changed_master_check)
        ...
        for i in range(n_channels):
            ...
            check_columns = tkinter.Checkbutton(internal_windows,
                                                variable=self.check_boxes[-1])
            self.check_boxes[-1].set(1)
            self.check_boxes[-1].trace_id = self.check_boxes[-1].trace_add('write', checks_changed)
        ...
        for i in range(n_channels, len(Color_pointers)):
            ...
            internal_windows.pack(side=tkinter.TOP)
            label = tkinter.Label(
                    internal_windows, text=analyze_index[i-n_channels])
